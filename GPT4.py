...
class Graph:
    def __init__(self):
        self.vertices = []  # 所有顶点（单词）
        self.edges = defaultdict(list)  # 邻接表
        self.weights = {}  # 边的权重
...
def dijkstra(graph, start, end):
    ...
    def reconstruct_paths(current):
        ...
        for predecessor in previous[current]:
            for path in reconstruct_paths(predecessor):
                paths.append(path + [current])
        return paths

    paths = reconstruct_paths(end)
    total_length = distances[end]  # 所有最短路径的长度都是相同的，等于计算出的最短距离

    return paths, total_length


def calPageRank(graph, word, d=0.85, max_iterations=100, tol=1e-6):
    """改进版：根据词频初始化PR值（不使用numpy）"""
    ...
    for from_v in graph.edges:
        from_idx = vertex_to_index[from_v]
        out_degree = len(graph.edges[from_v])
        if out_degree > 0: # 避免除以零
            for to_v in graph.edges[from_v]:
                to_idx = vertex_to_index[to_v]
                adj_matrix[to_idx][from_idx] += 1.0 / out_degree
        else:
            # 处理悬挂节点：假设它们均匀地链接到所有节点
            for to_idx in range(n):
                 adj_matrix[to_idx][from_idx] = 1.0 / n
    ...
    for iteration in range(max_iterations):
        prev_pr = pr[:] # 使用切片复制列表
        new_pr = [(1 - d) / n] * n # 基本概率
        for i in range(n): # 遍历每个节点 i (目标节点)
            rank_sum = 0.0
            for j in range(n): # 遍历每个节点 j (源节点)
                # adj_matrix[i][j] 表示从 j 到 i 的转移概率（或其贡献）
                rank_sum += adj_matrix[i][j] * prev_pr[j]
            new_pr[i] += d * rank_sum
        pr = new_pr

        # 检查收敛
        delta = sum(abs(pr[i] - prev_pr[i]) for i in range(n))
        if delta < tol:
            # print(f"Converged after {iteration + 1} iterations.") # 可选：打印收敛信息
            break
    # else: # 可选：如果循环正常结束（未break）
